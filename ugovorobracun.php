...
*
* @file         ugovorobracun.php
* @package       
* @subpackage   
*
* @description  Contract calculation
*
...
if (isset($_POST["rule"])) {
  ...
  if ($correct) {
    ...
    $net_sum = $res['net_sum'];
    $query = mysql_query(sprintf("SELECT * FROM oszcontractruledef WHERE IDInvoiceRule = %d ORDER BY sort", $rule)) or die(mysql_error());
    // dump(sprintf("SELECT * FROM oszcontractruledef WHERE IDInvoiceRule = %d ORDER BY sort", $rule));
    $rules = array();
    //$rules[0] = array();
        ...
        if ($res["input"] > 0 && $res["inputY"] == 0 && $res["inputZ"] == 0) {        // one operant
          switch ($res["operator"]) {
            ...
            case "/":
              ...
              break;
            default:
              $rules[$res['ID']]['result'] = $rules[$res["input"]]['result'];
              break;
          }
    ...
    //$query = mysql_query(sprintf("SELECT * FROM oszcontract$_currentyear WHERE IDContract = %d", $IDContract)) or die(mysql_error());
    //$res = mysql_fetch_array($query);
    //$IDInvoice = 0;
    //if (isset($res["ID"])) $IDInvoice = $res["ID"]; 
    if ($IDContract > 0) {
      // clean previous billing
      $query = mysql_query(sprintf("DELETE FROM oszcontractruleitem$_currentyear WHERE IDContract = %d", $IDContract)) or die(mysql_error());
      //$query = mysql_query(sprintf("DELETE FROM oszinvoiceitem$_currentyear WHERE IDInvoice = %d", $IDInvoice)) or die(mysql_error());
      $sql = "UPDATE oszcontract$_currentyear SET net=%f, tax=%f, contribute=%f, claimsum=%f, sum=%f, rule=%d WHERE ID=%d";
      mysql_query(sprintf($sql,
        $net_sum,
        $rules[$GLOBALS["agec"][$rule]["tax"]]['result'],
        $contribute,
        $rules[$GLOBALS["agec"][$rule]["claimsum"]]['result'],
        $rules[$GLOBALS["agec"][$rule]["sum"]]['result'],
        $rule,
        $IDContract
      )) or die(mysql_error());
    }
    $sql = "INSERT INTO oszcontractruleitem$_currentyear (IDContract, IDInvoiceruledef, net, value) VALUES (%d, %d, %d, %f)";
    ...
    $sql = "UPDATE oszcontractitem$_currentyear SET net=%f, value=%f, pio=%f, health=%f, insurance=%f, bruto=%f WHERE IDContract = %d AND IDMember = %d";
    while ($res = mysql_fetch_array($query)) {
      mysql_query(sprintf($sql, 
        $res["net"],
        $res["net"] * $GLOBALS["agec"][$rule]['transform'],
        $res["net"] * $GLOBALS["agec"][$rule]['pio'],
        $res["net"] * $GLOBALS["agec"][$rule]['health'],
        $res["net"] * $GLOBALS["agec"][$rule]['insurance'],
        $res["net"] * $GLOBALS["agec"][$rule]['bruto'],
        $IDContract,
        $res["IDMember"]
      )) or die(mysql_error());
    }
    ...
    $query    = mysql_query(sprintf("SELECT * FROM oszcontract$_currentyear WHERE ID=%d", $IDContract)) or die(mysql_error());
        ...
        <td colspan="2">
          <table cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr>
              ...
              <td style="border:0px none;" align="center"><a href="./pregledugobracun.php?IDContract=<?=$IDContract;?>&from=c" target="_blank">Pregled obračuna<br /><img src="./images/form/bill.jpg" alt="Pregled obračuna" title="Pregled obračuna" /></a></td>
            </tr>
          </table>
        </td>
      </tr>
