...
*
* @file         parametarstampaugunos.php
* @package       
* @subpackage   
*
* @description  Edit contract print to pdf parameters
*
...
if (isset($_GET["sort"]) && $id > 0) {
  $query = mysql_query(sprintf("SELECT * FROM oszcontractruleprt WHERE ID=%d",$id)) or die(mysql_error());
  $res   = mysql_fetch_array($query);
  $IDInvoiceRule = $res["IDInvoiceRule"];
  $sort  = $res["sort"];
  if ($_GET["sort"] == "up") {  // up
    mysql_query(sprintf("UPDATE oszcontractruleprt SET sort=%d WHERE IDInvoiceRule=%d AND sort=%d", $sort, $IDInvoiceRule, $sort-1)) or die(mysql_error());
    mysql_query(sprintf("UPDATE oszcontractruleprt SET sort=%d WHERE ID=%d", $sort-1, $id)) or die(mysql_error());
  } else {                      // down
    mysql_query(sprintf("UPDATE oszcontractruleprt SET sort=%d WHERE IDInvoiceRule=%d AND sort=%d", $sort, $IDInvoiceRule, $sort+1)) or die(mysql_error());
    mysql_query(sprintf("UPDATE oszcontractruleprt SET sort=%d WHERE ID=%d", $sort+1, $id)) or die(mysql_error());
  }
...
if ($id > 0) { 
  ...
  $query = mysql_query(sprintf("SELECT * FROM oszcontractruleprt WHERE IDInvoiceRule=%d AND invoice ORDER BY sort", $id)) or die(mysql_error());
  $prt = array();
  while ($res = mysql_fetch_array($query)) {
    $prt[$res["ID"]] = $res["name"];
  }

  $query = mysql_query(sprintf("SELECT * FROM oszcontractruleprt WHERE IDInvoiceRule=%d AND NOT invoice ORDER BY sort", $id)) or die(mysql_error());
