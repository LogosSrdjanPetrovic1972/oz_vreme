...
*
* @file         pdfugovorlist.php
* @package       
* @subpackage   
*
* @description  Print contract payment list to pdf
*
...
class PDF extends FPDF
{
  ...
  function AddTable($header,$data)
  {
      ...
      $this->Ln();
      //Data
      $this->SetFont('Arial','B',PDF_SMALLER);
      foreach($data as $row)
      {
...
if (isset($_GET["IDContract"]) && (int)$_GET["IDContract"] > 0) { // print to pdf
  ...
  $pdf->Cell(0,10,'Smed. Palanka, dana '._db_date_to_date($contract["contractdate"]),0,0);

  $pdf->Ln(10);
  $pdf->Line(10, $pdf->GetY(), 200, $pdf->GetY());
  $pdf->SetFont('Arial','',PDF_MIDDLE);
  $pdf->Cell(0,10,'Spisak uz zbirni ugovor broj : '.$contract["ID"],0,0,'C');
  $pdf->Ln(10);
  $pdf->Line(10, $pdf->GetY(), 200, $pdf->GetY());
  $pdf->SetFont('Arial','',PDF_SMALL);
  $header=array('Red.Br.', 'Šifra','Prezime i ime','Iznos');
  $pdf->Ln(10);
  ...
  while ($res = mysql_fetch_array($contractlist)) {
    $suma += $res['bruto'];
    $data[] = array($i.'.', $res['IDMember'], $pdf->StringConvert($res['surname'].' '.$res['name']), $res['bruto']);
    //echo $res['surname'].' '.$res['name'].'<br/>';
    $i++;
  }
