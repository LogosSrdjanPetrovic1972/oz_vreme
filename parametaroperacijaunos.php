...
*
* @file         parametaroperacijaunos.php
* @package       
* @subpackage   
*
...
//  die;
  ...
  if ($ID > 0) {  // update
    $sql = "UPDATE oszinvoiceruledef SET name='%s', sort=%d, invoice=%d, report=%d, operator='%s', input=%d, inputVal=%f, inputY=%d, operatorY='%s', inputZ=%d, contributesum=%d, inputnet=%d, control=%d WHERE ID=%d";
    ...
    if ($IDInvoiceRule > 0) {
      $sql = "INSERT INTO oszinvoiceruledef (IDInvoiceRule, name, sort, invoice, report, operator, input, inputVal, inputY, operatorY, inputZ, contributesum, inputnet, control)";
...
if ($ID > 0) {
  $query = mysql_query(sprintf("SELECT * FROM oszinvoiceruledef WHERE ID=%d",$ID)) or die(mysql_error());
...
if (isset($_GET["id"]) && isset($_GET["add"]) && $_GET["add"] == 'del') {
  // delete action
  mysql_query("DELETE FROM oszinvoiceruledef WHERE ID=$ID") or die(mysql_error());
  header("location: parametarunos.php?id=$IDInvoiceRule");
}
...
if ($IDInvoiceRule > 0) {
  ...
  $query = mysql_query(sprintf("SELECT * FROM oszinvoiceruledef WHERE IDInvoiceRule=%d ORDER BY sort", $IDInvoiceRule)) or die(mysql_error());
