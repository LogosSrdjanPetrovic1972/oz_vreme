...
*
* @file         parametarpregled.php
* @package       
* @subpackage   
*
* @description  View invoice parameter
*
* @history      13.06.2013. ; srdjanp ; Initial revision
*
*/
...
if (isset($_GET["sort"]) && $id > 0) {
  ...
  $sort  = $res["sort"];
  if ($_GET["sort"] == "up") {  // up
    mysql_query(sprintf("UPDATE oszinvoiceruledef SET sort=%d WHERE IDInvoiceRule=%d AND sort=%d", $sort, $IDInvoiceRule, $sort-1)) or die(mysql_error());
    mysql_query(sprintf("UPDATE oszinvoiceruledef SET sort=%d WHERE ID=%d", $sort-1, $id)) or die(mysql_error());
  } else {                      // down
    mysql_query(sprintf("UPDATE oszinvoiceruledef SET sort=%d WHERE IDInvoiceRule=%d AND sort=%d", $sort, $IDInvoiceRule, $sort+1)) or die(mysql_error());
    mysql_query(sprintf("UPDATE oszinvoiceruledef SET sort=%d WHERE ID=%d", $sort+1, $id)) or die(mysql_error());
  }
